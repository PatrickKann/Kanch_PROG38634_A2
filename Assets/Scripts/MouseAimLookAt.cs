...
public class MouseAimLookAt : MonoBehaviour {
    ...
    public void LateUpdate(){
		...
		if (weapon)
		{
			Vector3 fwd = weapon.TransformDirection(Vector3.forward * 1000);
	...
	void SpawnImpact(Transform gun, RaycastHit hit)//Transform hit)
	{
		GameObject gunImpact;
		gunImpact = Instantiate(impact, hit.point, Quaternion.identity) as GameObject;
		gunImpact.name = impact.name;
	}
}
